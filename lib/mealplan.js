...
const DAYS = 14;
const LOOKBACK = 2; // don't repeat a dish within this many days
...
function topUpProtein(dayMeals, pools, proteinTarget, kcalTargets) {
  let attempts = 0;
  ...
  while (attempts < 6) {
    ...
    const pool = pools[slot].filter(d => !usedTodayIds.has(d.id) && kcalOf(d) <= kcalCeiling);
...
export function generateMealPlan(t, veg) {
  ...
  for (let day = 1; day <= DAYS; day++) {
    ...
    dayMeals = topUpProtein(dayMeals, pools, t.proteinG, kcalTargets);
