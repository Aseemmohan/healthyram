...
export function selectDishes({ veg, goal, baselineResult, eats = [] }) {
  ...
  const defaultTier = far ? 2 : some ? 1 : 1;

  return pool
    .filter(d => eats.length === 0 || eats.includes(d.id))
    .map(d => {
      // Push a tier harder where one component dominates — the swap is cheap there.
      const worst = Math.max(...d.components.map(c => c.kcal));
      const share = worst / d.components.reduce((s, c) => s + c.kcal, 0);
      let tier = defaultTier;
      if (share > 0.5 && far) tier = Math.min(3, tier + 1);
      if (goal === "maintain") tier = 1;

      return {
        id: d.id,
        name: d.name,
        recommendedTier: tier,
        tier: d.tiers.find(t => t.level === tier),
        headline: d.headline,
      };
    })
